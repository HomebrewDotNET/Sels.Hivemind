...
namespace Sels.HiveMind.RequestHandlers.Background
{
    /// <summary>
    ...
    public class BackgroundJobRetryHandler : IBackgroundJobStateElectionRequestHandler
    {
        ...
        {
            if(request.ElectedState is FailedState failedState)
            {
                ...
                if(currentRetryCount < options.MaxRetryCount)
                {
                    ...
                    if (failedState.Exception == null || !options.IsFatal(failedState.Exception))
                    {
