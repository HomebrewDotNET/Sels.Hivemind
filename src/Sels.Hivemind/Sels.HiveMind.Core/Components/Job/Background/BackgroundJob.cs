...
namespace Sels.HiveMind.Job.Background
{
    ...
    public class BackgroundJob : BaseJob<IBackgroundJobClient, IBackgroundJobService, ILockedBackgroundJob, BackgroundJobChangeLog, IBackgroundJobChangeTracker, BackgroundJobStorageData, IBackgroundJobState, JobStateStorageData, IBackgroundJobAction>, ILockedBackgroundJob
    {
        ...
        public async Task SaveChangesAsync(IStorageConnection connection, bool retainLock, CancellationToken token = default)
        {
            ...
            Logger.Log($"Saving changes made to background job <{HiveLog.Job.IdParam}> in environment <{EnvironmentParam}>", Id, Environment);
            // Validate lock
            ...
            Logger.Log($"Saved changes made to background job <{HiveLog.Job.IdParam}> in environment <{EnvironmentParam}>", Id, Environment);
        }
        /// <inheritdoc/>
        public async Task SaveChangesAsync(bool retainLock, CancellationToken token = default)
        {
            Logger.Debug($"Opening new connection to storage in environment <{EnvironmentParam}> for background job <{HiveLog.Job.IdParam}> to save changes", Environment, Id);
        ...
        public async Task<bool> SetSystemDeletedAsync(IStorageConnection connection, string reason = null, CancellationToken token = default)
        {
            ...
            {
                Logger.Warning($"Could not elect deleted state for job <{HiveLog.Job.IdParam}> in environment <{EnvironmentParam}>.", Id, Environment);
        ...
        protected override bool CanCancelJob()
        {
            ...
            {
                Logger.Log($"Background job <{HiveLog.Job.IdParam}> in environment <{EnvironmentParam}> is not in a valid state to be cancelled. Current state is <{HiveLog.Job.StateParam}>", Id, Environment, State.Name);
                return false;
            }
            return true;
        }
