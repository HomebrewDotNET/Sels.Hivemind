...
namespace Sels.HiveMind.Templates.Client
{
    ...
    public abstract class BaseJobClient<TService, TReadOnlyJob, TLockedJob, TStorageData, TState, TStateStorageData> : BaseClient, IJobClient<TReadOnlyJob, TLockedJob>
        ...
        where TService : IJobService<TStorageData, TState, TStateStorageData>
    {
        ...
        public async Task<IClientQueryResult<TLockedJob>> GetTimedOutAsync(IStorageConnection connection, string requester, int limit = 100, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                return CreateQueryResult(connection.Environment, jobs, lockedJobs.LongLength, true);
            }
            catch (Exception ex)
            {
        ...
        protected abstract IClientQueryResult<TLockedJob> CreateQueryResult(string environment, IReadOnlyList<TLockedJob> jobs, long total, bool isTimedOut);
        /// <summary>
